...
public class SalesTargetsController : ControllerBase
{
    ...
    {
        try { var result = await _service.GetByUserIdAsync(userId, fiscalYear); return Ok(new ApiResponse<IEnumerable<SalesTargetDto>> { Success = true, Data = result }); }
    ...
    {
        try { var result = await _service.GetByTeamIdAsync(teamId, fiscalYear); return Ok(new ApiResponse<IEnumerable<SalesTargetDto>> { Success = true, Data = result }); }
